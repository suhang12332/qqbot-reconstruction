...
func (e *PluginEngine) SetPluginStatus(name string, status bool) {
...
func (e *PluginEngine) SetWhiteList(name string, whiteList []string, mode string) {
	...
	switch mode {
	case variable.ADD:
		plugin.SetWhiteList(removeRepeatedElement(append(wl, whiteList...)))
	case variable.REMOVE:
		if len(wl) != 0 {
			newWl := removeElement(whiteList, wl)
			if len(newWl) != len(wl) {
...
func updatePlugin(plugin *Plugin, info *variable.PluginInfo) bool {
	...
	if !isStringArraysEqual((*plugin).GetWhiteList(), info.Whitelist) {
