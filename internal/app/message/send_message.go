...
func (receive *Receive) SubscribeFailedMsgTips(name string) *Send {
	return receive.Tips(fmt.Sprintf(` "%s"订阅消息发送失败`, name))
}

func (receive *Receive) SubscribeFailedTips(name string) *Send {
	return receive.Tips(fmt.Sprintf(` "%s"订阅失败，指定起始执行时间后，执行周期最少为24小时`, name))
}
