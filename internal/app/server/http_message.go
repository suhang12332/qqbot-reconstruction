...
func (send *Send) queryCloudSong(info string) variable.CloudSong {
	...
	return handle.HttpHandler(http.MethodGet, urls, nil, &variable.CloudSong{}, header, variable.JSON, false,nil,false,nil)
}
// queryCloudSong
// @description: 获取阿里云盘
// @param info 搜索信息
...
func (send *Send) queryAliDriver(info string) variable.AliResponse {
	...
	return handle.HttpHandler(http.MethodGet, urls, nil, &variable.AliResponse{}, header, variable.JSON, false,nil,true, decodeBase64)
}
...
func (send *Send) queryMagnet(info string) variable.MagnetResult {
	...
	return handle.HttpHandler(http.MethodGet,urls,nil,&variable.MagnetResult{},nil,variable.HTML,false, magnet,false,nil)
}
